<?php
  ...
  if (isset($_SESSION['accountCreated']) && $_SESSION['accountCreated'] == true) {
    echo '<script>window.onload = function() { $("#modalMessage").html("Account Created Successfully!"); $("#messageModal").modal("show"); }</script>';
    session_unset();
    session_destroy();
  }
  ...
  if(isset($_POST['logInBtn'])){
        ...
        $num = 0;
        $query = "SELECT username, COUNT(*) AS num FROM users WHERE username = '$username' AND password = '$password';";
        ...
        if ($num == 1) {
            ...
            $_SESSION['residents_id'] = $residents_id;

            header("Location: /portfolio/index.html"); 
            exit();
        } else {
            echo '<script>window.onload = function() { $("#modalMessage").html("Invalid Log In!"); $("#messageModal").modal("show"); }</script>';
        }
    }
...
<html lang="en">
  ...
  <body class="body">
    <div class="container logIn">
      <form method='post'>
        <h3 class="text-center" style="font-weight: 700; margin-top: 50px;">LOG IN</h3>
        ...
        <div>
          <button style="margin-top: 40px;" type="submit" name="logInBtn" class="btn btn-outline-warning button">LOG - IN</button>
        </div>

        <div>
          <button style="margin-top: 20px;" onclick="location.href = 'createAccount.php';" type="button" class="btn btn-outline-warning button">CREATE ACCOUNT</button>
        </div>
      </form>
